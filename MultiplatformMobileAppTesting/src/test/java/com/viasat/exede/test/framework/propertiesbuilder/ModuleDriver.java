...
public class ModuleDriver {
    ...
    public static void main(String args[]) throws IOException {
        List<Class> classes = ClassReader.getClassesInPackage("com.viasat.exede.mobile.test.pages");

        for (Class clazz : classes) {
            File createdPropertyFile = PropertyUtil.createPropertyFileObjectForClass(clazz);
            List<Field> mobileElementFields = ClassReader.parseClassFields("io.appium.java_client.MobileElement", clazz);
